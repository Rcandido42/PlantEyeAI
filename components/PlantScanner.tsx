...
const PlantScanner: React.FC<PlantScannerProps> = ({ onResult, isAnalyzing, setIsAnalyzing, deviceId, onGeminiError }) => {
  ...
  const captureAndAnalyze = useCallback(async () => {
    ...
    if (ctx) {
      ...
      if (!navigator.onLine) {
        onResult({ species: 'Diagnóstico Pendente', status: PlantStatus.UNKNOWN, healthStatus: 'Desconhecido' as AnalysisResult['healthStatus'], threatDetected: 'nenhuma', severityLevel: 0, forestryRisk: 'Baixo', recommendations: ['Imagem guardada localmente.'], recommendation: '', raizReference: '', summary: 'Captura offline. GPS registado.', lightLevel: LightLevel.UNKNOWN, confidence: 0, isInvasive: false, invasiveSpecies: null, isPending: true, imageBase64: base64 }, dataUrl, coords);
        setOfflineSaved(true); setTimeout(() => setOfflineSaved(false), 4000); setIsAnalyzing(false); return;
      }
      try { const result = await analyzePlantImage(base64); onResult(result, dataUrl, coords); } catch (err) { if (!onGeminiError?.(err)) setError('Falha ao analisar a imagem.'); } finally { setIsAnalyzing(false); }
    }
