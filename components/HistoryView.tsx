...
interface HistoryViewProps {
  history: HistoryItem[]; onClearHistory: () => void; onDeleteItem: (id: string) => void;
  onMarkRemoved: (id: string) => void; onReanalyze: (item: HistoryItem) => Promise<void>; isOnline: boolean;
}
...
const HistoryView: React.FC<HistoryViewProps> = ({ history, onClearHistory, onDeleteItem, onMarkRemoved, onReanalyze, isOnline }) => {
  ...
  const formatDate = (timestamp: number) => new Intl.DateTimeFormat('pt-PT', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }).format(new Date(timestamp));
  ...
  if (history.length === 0) {
    return (
      <div className="text-center py-20 bg-white rounded-[3rem] border border-dashed border-emerald-200">
        <div className="w-16 h-16 bg-emerald-50 rounded-full flex items-center justify-center mx-auto mb-4"><History className="w-8 h-8 text-emerald-200" /></div>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-emerald-600/50 font-medium mt-0.5">{filteredHistory.length} de {history.length} diagnóstico{history.length !== 1 ? 's' : ''}</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => exportAllPDF(filteredHistory)} className="flex items-center gap-1.5 px-3 py-2 bg-[#064E3B] text-white rounded-full text-xs font-black hover:bg-emerald-800 transition-colors" title="Exportar PDF"><FileDown className="w-4 h-4" />PDF</button>
          <button onClick={() => setShowFilters(!showFilters)} className={`relative p-2 rounded-full transition-colors ${showFilters ? 'bg-emerald-600 text-white' : 'text-emerald-600 hover:bg-emerald-50'}`} title="Filtros">
            <Filter className="w-5 h-5" />
            {activeFiltersCount > 0 && <span className="absolute -top-1 -right-1 w-4 h-4 bg-rose-500 text-white text-[9px] font-black rounded-full flex items-center justify-center">{activeFiltersCount}</span>}
          </button>
          <button onClick={() => { if (window.confirm('Tem a certeza que quer apagar todos os registos de campo?')) onClearHistory(); }} className="p-2 text-rose-500 hover:bg-rose-50 rounded-full transition-colors" title="Limpar Registos"><Trash2 className="w-5 h-5" /></button>
        </div>
      </div>
      {showFilters && (
        <div className="bg-white rounded-[2rem] border border-emerald-100 p-5 space-y-4 shadow-sm">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {([{ value: 'ALL', label: 'Todos' }, { value: 'HEALTHY', label: 'Saudável' }, { value: 'THIRSTY', label: 'Deficit Hídrico' }, { value: 'SICK', label: 'Anomalia' }] as { value: StatusFilter; label: string }[]).map(({ value, label }) => (
                <button key={value} onClick={() => setStatusFilter(value)} className={`px-3 py-1.5 rounded-full text-xs font-bold transition-colors ${statusFilter === value ? 'bg-[#064E3B] text-white' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100'}`}>{label}</button>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {([{ value: 'ALL', label: 'Todos' }, { value: 'TODAY', label: 'Hoje' }, { value: 'WEEK', label: 'Esta semana' }, { value: 'MONTH', label: 'Este mês' }] as { value: DateFilter; label: string }[]).map(({ value, label }) => (
                <button key={value} onClick={() => setDateFilter(value)} className={`px-3 py-1.5 rounded-full text-xs font-bold transition-colors ${dateFilter === value ? 'bg-[#064E3B] text-white' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100'}`}>{label}</button>
              ))}
            </div>
          </div>
          {activeFiltersCount > 0 && <button onClick={() => { setStatusFilter('ALL'); setDateFilter('ALL'); }} className="flex items-center gap-1.5 text-xs text-rose-500 font-bold hover:text-rose-700 transition-colors"><X className="w-3.5 h-3.5" />Limpar filtros</button>}
        </div>
      )}
      {filteredHistory.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-[2rem] border border-dashed border-emerald-200"><p className="text-emerald-900 font-bold mb-1">Sem resultados</p><p className="text-emerald-600/70 text-sm">Tenta ajustar os filtros aplicados.</p></div>
      ) : (
      ...
      {selectedItem && <AnalysisResultView result={selectedItem} image={selectedItem.imageUrl} onClose={() => setSelectedItem(null)} />}
    </div>
  );
};
