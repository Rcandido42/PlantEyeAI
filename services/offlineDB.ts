...
export interface DiagnosticoPendente { id: string; imageBlob: Blob; coords: GpsCoords | null; timestamp: string; userId: string; status: DiagnosticoStatus; retryCount: number; }
...
export function openDB(): Promise<IDBDatabase> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = (event) => { _db = (event.target as IDBOpenDBRequest).result; resolve(_db!); };
...
export async function saveDiagnosticoPendente(payload: Omit<DiagnosticoPendente, 'status' | 'retryCount'>): Promise<DiagnosticoPendente> {
  const item: DiagnosticoPendente = { ...payload, status: 'pendente', retryCount: 0 };
...
export async function getPendentes(): Promise<DiagnosticoPendente[]> {
  ...
  const pendentes = await promisify<DiagnosticoPendente[]>(index.getAll('pendente') as IDBRequest<DiagnosticoPendente[]>);
  const comErro = await promisify<DiagnosticoPendente[]>(index.getAll('erro') as IDBRequest<DiagnosticoPendente[]>);
  return [...pendentes, ...comErro.filter((d) => d.retryCount < 3)];
}

export async function updateStatus(id: string, status: DiagnosticoStatus, incrementRetry = false): Promise<void> {
  const store = await getStore('readwrite');
  const item = await promisify<DiagnosticoPendente>(store.get(id) as IDBRequest<DiagnosticoPendente>);
  if (!item) return;
  item.status = status;
